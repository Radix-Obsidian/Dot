...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.DOT_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.DOT_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.DOT_NODE_OPTIONS_READY = "1";
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[dot] ${parsed.error}`);
    process.exit(2);
  }
