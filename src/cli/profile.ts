...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.dot${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.DOT_PROFILE = profile;

  const stateDir = env.DOT_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.DOT_STATE_DIR?.trim()) {
    env.DOT_STATE_DIR = stateDir;
  }

  if (!env.DOT_CONFIG_PATH?.trim()) {
    env.DOT_CONFIG_PATH = path.join(stateDir, "dot.json");
  }

  if (profile === "dev" && !env.DOT_GATEWAY_PORT?.trim()) {
    env.DOT_GATEWAY_PORT = "19001";
  }
}
