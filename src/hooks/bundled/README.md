...
This directory contains hooks that ship with dot. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.dot.ai/hooks/soul-evil
...
Hooks can be configured in `~/.dot/dot.json`:
...
2. Restart gateway: `pkill -9 -f 'dot.*gateway' && pnpm dot gateway`
3. Enable the hook: `dot hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.dot.ai/hooks
