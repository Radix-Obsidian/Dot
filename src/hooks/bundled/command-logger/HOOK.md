...
homepage: https://docs.dot.ai/hooks#command-logger
metadata:
  {
    "dot":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.dot/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.dot/logs/commands.log ~/.dot/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/dot`:
   ```
   /home/username/.dot/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
