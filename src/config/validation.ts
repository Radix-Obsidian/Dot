...
import type { DotConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: DotConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: DotConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = DotSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as DotConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as DotConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as DotConfig)),
    ),
  };
}
