...
const NEW_STATE_DIRNAME = ".dot";
const CONFIG_FILENAME = "dot.json";
 ...
 * Default: ~/.dot/dot.json (or $DOT_STATE_DIR/dot.json)
 */
 ...
 * Default: os.tmpdir()/dot-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `dot-${uid}` : "dot";
