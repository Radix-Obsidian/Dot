...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/dot`)
...
If the page fails to load, start the Gateway first: `dot gateway`.
...
you revoke it with `dot devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.dot/dot.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. dot
...
```bash
dot gateway --bind tailnet --token "$(openssl rand -hex 32)"
```
...
dot **blocks** Control UI connections without device identity.
...
```bash
dot_CONTROL_UI_BASE_PATH=/dot/ pnpm ui:build
```
