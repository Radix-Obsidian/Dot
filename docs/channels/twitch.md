...
read_when:
  - Setting up Twitch chat integration for dot
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. dot connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
dot plugins install @dot/twitch
```

Local checkout (when running from a git repo):

```bash
dot plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `dot_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "dot", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use dot_TWITCH_ACCESS_TOKEN env var)
...
```bash
dot_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
