...
dot can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, dot retries the same message as plain text.

## Commands (native + custom)

dot registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; dot does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `dot logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). dot can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. dot:
...
Private chats can include `message_thread_id` in some edge cases. dot keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
- Approve via:
  ...
  - `dot pairing approve telegram <CODE>`
...
2. Run `dot logs --follow` and look for `from.id`.
...
dot defaults to audio files for backward compatibility.
...
dot supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, dot handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, dot caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.dot/telegram/sticker-cache.json`
...
dot uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, dot:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by dot)
...
- Example: `dot message send --channel telegram --target 123456789 --message "hi"`.
...
- `dot channels status` shows a warning when config expects unmentioned group messages.
- `dot channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `dot logs --follow` (look for "skipping group message")
...
- Upgrade to a dot build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
