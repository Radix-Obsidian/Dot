...
Tlon is a decentralized messenger built on Urbit. dot connects to your Urbit ship and can
...
```bash
dot plugins install @dot/tlon
```

Local checkout (when running from a git repo):

```bash
dot plugins install ./extensions/tlon
```
...
Use these with `dot message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, dot replies in-thread.
