...
```bash
dot plugins install @dot/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
dot plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "dot" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `dot pairing list nextcloud-talk`
  - `dot pairing approve nextcloud-talk <CODE>`
