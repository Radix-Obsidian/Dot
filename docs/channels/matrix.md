...
Matrix is an open, decentralized messaging protocol. dot connects as a Matrix **user**
...
```bash
dot plugins install @dot/matrix
```

Local checkout (when running from a git repo):

```bash
dot plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `dot plugins install @dot/matrix`
   - From a local checkout: `dot plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: dot calls the same
     login endpoint, stores the access token in `~/.dot/credentials/matrix/credentials.json`,
...
- On first connection, dot requests device verification from your other sessions.
...
`~/.dot/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `dot pairing list matrix`
  - `dot pairing approve matrix <CODE>`
...
- On startup, dot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
