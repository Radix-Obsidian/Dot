...
3. Set tokens for dot and start the gateway.
...
7. Slash Commands → create `/dot` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
dot can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, dot falls back to the user token.
...
```json
{
  "display_information": {
    "name": "dot",
    "description": "Slack connector for dot"
  },
  "features": {
    "bot_user": {
      "display_name": "dot",
    ...
    "slash_commands": [
      {
        "command": "/dot",
        "description": "Send a message to dot",
        "should_escape": false
      }
    ]
  },
...
By default, dot replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, dot infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `dot pairing approve slack <code>`.
...
- On startup, dot resolves channel/user names in allowlists to IDs (when tokens allow)
