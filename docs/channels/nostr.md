...
read_when:
  - You want dot to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables dot to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`dot onboard`) and `dot channels add` list optional channel plugins.
...
```bash
dot plugins install @dot/nostr
```

Use a local checkout (dev workflows):

```bash
dot plugins install --link <path-to-dot>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "dot@example.com",
        "lud16": "dot@example.com"
      }
    }
  }
}
