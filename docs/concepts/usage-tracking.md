...
- `/usage cost` in chats: local cost summary aggregated from dot session logs.
- CLI: `dot status --usage` prints a full per-provider breakdown.
- CLI: `dot channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
