...
read_when:
  - You want to understand what “context” means in dot
...
“Context” is **everything dot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (dot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How dot builds the system prompt

The system prompt is **dot-owned** and rebuilt each run. It includes:
...
By default, dot injects a fixed set of workspace files (if present):
