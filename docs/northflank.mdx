...
Deploy dot on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy dot](https://northflank.com/stacks/deploy-dot) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy dot now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the dot template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the dot service.
8. Open the public dot URL and complete setup at `/setup`.
9. Open the Control UI at `/dot`.

## What you get

- Hosted dot Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/dot`
