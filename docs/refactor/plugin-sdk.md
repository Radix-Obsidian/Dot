...
- Publish as `dot/plugin-sdk` (or export from core under `dot/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `dotPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: dotConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: dotConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: dotConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: dotConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: dotConfig): string;
  };
};
```
...
- Introduce `dot/plugin-sdk`.
- Add `api.runtime` to `dotPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `dotRuntime: ">=2026.2.0"`).
