...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in dot
...
Running dot via Node with `tsx` fails at startup with:

```
[dot] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `dot status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch dot.mjs status
  ```
- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node dot.mjs status` works on Node 25.
