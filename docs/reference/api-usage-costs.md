...
dot features that can generate provider usage or paid API calls.
...
- `dot status --usage` and `dot channels list` show provider **usage windows**
...
dot can pick up credentials from:
...
`dot models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
