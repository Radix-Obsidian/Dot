...
Deploy dot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a
  href="https://render.com/deploy?repo=https://github.com/dot/dot"
  target="_blank"
  rel="noreferrer"
>
  Deploy to Render
...
repository configures everything needed to run dot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: dot_STATE_DIR
        value: /data/.dot
      - key: dot_WORKSPACE_DIR
        value: /data/workspace
      - key: dot_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/dot`.
...
If you use the original dot repository, Render will not auto-deploy your dot. To update it, run a manual Blueprint sync from the dashboard.
...
For dot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any dot host.
