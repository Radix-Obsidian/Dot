...
read_when:
  - You want to understand `dot.ai/install.sh`
...
dot ships two installer scripts (served from `dot.ai`):

- `https://dot.ai/install.sh` — “recommended” installer (global npm install by default; can also install from a GitHub checkout)
- `https://dot.ai/install-cli.sh` — non-root-friendly CLI installer (installs into a prefix with its own Node)
- `https://dot.ai/install.ps1` — Windows PowerShell installer (npm by default; optional git install)
...
```powershell
& ([scriptblock]::Create((iwr -useb https://dot.ai/install.ps1))) -?
```

If the installer completes but `dot` is not found in a new terminal, it’s usually a Node/npm PATH issue. See: [Install](/install#nodejs--npm-path-sanity).
...
- If upgrading an existing install: runs `dot doctor --non-interactive` (best effort).
- For git installs: runs `dot doctor --non-interactive` after install/update (best effort).
...
```bash
SHARP_IGNORE_GLOBAL_LIBVIPS=0 curl -fsSL https://dot.ai/install.sh | bash
```

### Discoverability / “git install” prompt

If you run the installer while **already inside a dot source checkout** (detected via `package.json` + `pnpm-workspace.yaml`), it prompts:

- update and use this checkout (`git`)
- or migrate to the global npm install (`npm`)

In non-interactive contexts (no TTY / `--no-prompt`), you must pass `--install-method git|npm` (or set `dot_INSTALL_METHOD`), otherwise the script exits with code `2`.
...
This script installs `dot` into a prefix (default: `~/.dot`) and also installs a dedicated Node runtime under that prefix, so it can work on machines where you don’t want to touch the system Node/npm.

Help:

```bash
curl -fsSL https://dot.ai/install-cli.sh | bash -s -- --help
```
...
- Choose install method:
  - `npm` (default): `npm install -g dot@latest`
  - `git`: clone/build a source checkout and install a wrapper script
- Runs `dot doctor --non-interactive` on upgrades and git installs (best effort).
...
```powershell
iwr -useb https://dot.ai/install.ps1 | iex -InstallMethod git
```

```powershell
iwr -useb https://dot.ai/install.ps1 | iex -InstallMethod git -GitDir "C:\\dot"
```
...
- **"dot" is not recognized**: your npm global bin folder is not on PATH. Most systems use
