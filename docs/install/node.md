...
read_when:
  - "You installed dot but `dot` is “command not found”"
...
dot’s runtime baseline is **Node 22+**.

If you can run `npm install -g dot@latest` but later see `dot: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
