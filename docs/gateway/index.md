...
- Replaces the legacy `gateway` command. CLI entry point: `dot gateway`.
...
- Config hot reload watches `~/.dot/dot.json` (or `dot_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__dot__/canvas/` from `~/.dot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `dot_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `dot_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `dot_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.dot.*` may still exist)
- Linux: `dot-gateway-<profile>.service`
- Windows: `dot Gateway (<profile>)`

Install metadata is embedded in the service config:

- `dot_SERVICE_MARKER=dot`
- `dot_SERVICE_KIND=gateway`
- `dot_SERVICE_VERSION=<version>`
...
```bash
dot --dev setup
dot --dev gateway --allow-unconfigured
...
- `dot_STATE_DIR=~/.dot-dev`
- `dot_CONFIG_PATH=~/.dot-dev/dot.json`
- `dot_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.dot/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `dot_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `dot_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:

- unique `gateway.port`
- unique `dot_CONFIG_PATH`
- unique `dot_STATE_DIR`
...
```bash
dot --profile main gateway install
dot --profile rescue gateway install
```

Example:

```bash
dot_CONFIG_PATH=~/.dot/a.json dot_STATE_DIR=~/.dot-a dot gateway --port 19001
dot_CONFIG_PATH=~/.dot/b.json dot_STATE_DIR=~/.dot-b dot gateway --port 19002
```
...
- `health` — full health snapshot (same shape as `dot health --json`).
...
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `dot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.dot.*` is cleaned up).
  - `dot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are dot profile services.
  ...
  - Cleanup: `dot gateway uninstall` (current service) and `dot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `dot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- dot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.dot.*` labels still unload cleanly).
- To stop it cleanly, use `dot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `dot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `dot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

dot installs a **systemd user service** by default on Linux/WSL2. We
...
`dot gateway install` writes the user unit. `dot doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/dot-gateway[-<profile>].service`:

```
[Unit]
Description=dot Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/dot gateway --port 18789
Restart=always
RestartSec=5
Environment=dot_GATEWAY_TOKEN=
...
```
systemctl --user enable --now dot-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/dot-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:

```
sudo systemctl daemon-reload
sudo systemctl enable --now dot-gateway[-<profile>].service
```
...
- `dot gateway health|status` — request health/status over the Gateway WS.
- `dot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `dot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `dot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `dot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `dot gateway` and the legacy TCP control port.
