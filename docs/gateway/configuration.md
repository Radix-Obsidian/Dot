---
summary: "All configuration options for ~/.dot/dot.json with examples"
read_when:
...
dot reads an optional **JSON5** config from `~/.dot/dot.json` (comments + trailing commas allowed).

If the file is missing, dot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.dot/workspace`). You usually only need a config to:
...
dot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `dot doctor`, `dot logs`, `dot health`, `dot status`, `dot service`, `dot help`).
- Run `dot doctor` to see the exact issues.
- Run `dot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `dot config set`. Keep a backup of `~/.dot/dot.json`.
...
```bash
dot gateway call config.get --params '{}' # capture payload.hash
dot gateway call config.apply --params '{
...
```bash
dot gateway call config.get --params '{}' # capture payload.hash
dot gateway call config.patch --params '{
...
{
  agents: {
    defaults: { workspace: "~/.dot/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@dot", "reisponde"] },
      },
    ],
  },
  channels: {
...
// ~/.dot/agents.json5
{
  ...
  list: [{ id: "main", workspace: "~/.dot/workspace" }],
}
```
...
{ "$include": "/etc/dot/base.json5" } // absolute
...
```json5
// ~/.dot/clients/mueller/agents.json5
[
...
```json5
// ~/.dot/clients/mueller/broadcast.json5
{
...
dot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, dot runs your login shell and imports only the missing expected keys (never overrides).
...
- `dot_LOAD_SHELL_ENV=1`
- `dot_SHELL_ENV_TIMEOUT_MS=15000`
...
dot stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.dot/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:

- `~/.dot/credentials/oauth.json` (or `$dot_STATE_DIR/credentials/oauth.json`)
...
- `~/.dot/agent/*` (migrated by `dot doctor` into `~/.dot/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `dot_OAUTH_DIR`
- Agent dir (default agent root override): `dot_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, dot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, dot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/dot/dot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/dot/dot.log`.
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.dot/credentials/whatsapp/biz
          // authDir: "~/.dot/credentials/whatsapp/biz",
        },
      },
    },
  },
...
- The legacy single-account Baileys auth dir is migrated by `dot doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [{ id: "main", groupChat: { mentionPatterns: ["@dot", "dot"] } }],
  },
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.dot/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.dot/agents/<agentId>/agent`.
...
```json5
{
  agents: {
    list: [
      {
        id: "personal",
        workspace: "~/.dot/workspace-personal",
        sandbox: { mode: "off" },
      },
    ],
  },
...
```json5
{
  agents: {
    list: [
      {
        id: "family",
        workspace: "~/.dot/workspace-family",
        sandbox: {
          mode: "all",
          scope: "agent",
          workspaceAccess: "ro",
...
```json5
{
  agents: {
    list: [
      {
        id: "public",
        workspace: "~/.dot/workspace-public",
        sandbox: {
          mode: "all",
          scope: "agent",
...
```json5
{
  agents: {
    list: [
      { id: "home", default: true, workspace: "~/.dot/workspace-home" },
      { id: "work", workspace: "~/.dot/workspace-work" },
    ],
  },
  bindings: [
...
- `commands.config: true` enables `/config` (reads/writes `dot.json`).
...
dot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["dot-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
dot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `dot plugins install @dot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
dot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
dot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, dot
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/dot" } },
}
```
...
When a file exceeds this limit, dot logs a warning and injects a truncated
...
message envelopes). If unset, dot uses the host timezone at runtime.
...
`[dot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[dot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[dot]"`, dot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, dot generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.dot/settings/tts.json",
      elevenlabs: {
...
dot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, dot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `dot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:dot`: all built-in dot tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.dot/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.dot/sandboxes",
        docker: {
          image: "dot-sandbox:bookworm-slim",
          containerPrefix: "dot-sbx-",
          workdir: "/workspace",
          readOnlyRoot: true,
        ...
        browser: {
          enabled: false,
          image: "dot-sandbox-browser:bookworm-slim",
          containerPrefix: "dot-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
dot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.dot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
dot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, dot writes/merges a `models.json` into
`~/.dot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. dot uses
...
- Shortcut: `dot onboard --auth-choice opencode-zen`.
...
Shortcut: `dot onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `dot onboard --auth-choice moonshot-api-key`.
...
- Set `KIMI_API_KEY` in the environment or use `dot onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `dot onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `dot onboard --auth-choice minimax-api`.
...
- Override the agent config root with `dot_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.dot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.dot/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.dot/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, dot stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.dot/skills` (workspace skills
...
from `~/.dot/extensions`, `<workspace>/.dot/extensions`, plus any
...
### `browser` (dot-managed browser)

dot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for dot and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (dot.app menubar, or `dot gateway`).
...
```json5
{
  browser: {
    ...
    profiles: {
      dot: { cdpPort: 18800, color: "#FF4500" },
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/dot" }
...
- When a connection comes from one of these IPs, dot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `dot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `dot_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `dot_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. dot
...
- dot.app watches `~/.dot/dot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.dot/dot.json` (or `dot_CONFIG_PATH`) and applies changes automatically.
...
- `~/.dot/dot.json` (or `dot_CONFIG_PATH`)
...
- `dot_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `dot --dev …` → uses `~/.dot-dev` + shifts ports from base `19001`
- `dot --profile <name> …` → uses `~/.dot-<name>` (port via config/env/flags)
...
```bash
dot_CONFIG_PATH=~/.dot/a.json \
dot_STATE_DIR=~/.dot-a \
dot gateway --port 19001
```
...
Gmail helper config (used by `dot webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "dot@gmail.com",
...
- Set `dot_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, dot defaults `serve.path` to `/` so
...
Default root: `~/.dot/workspace/canvas`  
Default port: `18793` (chosen to avoid the dot browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__dot__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__dot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.dot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.dot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.dot/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_dot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `dot` (advertises `dot.local`). Override with `dot_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_dot-gw._tcp` under `~/.dot/dns/` using the configured discovery domain (example: `dot.internal.`).
