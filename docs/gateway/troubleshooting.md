---
summary: "Quick troubleshooting guide for common dot failures"
read_when:
...
When dot misbehaves, here's how to fix it.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------ | ------------------------------------------------- |
| `dot status`                  | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview                       |
| `dot status --all`            | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail                                   | When you need to share a debug report             |
| `dot status --deep`           | Runs gateway health checks (incl. provider probes; requires reachable gateway)                         | When “configured” doesn’t mean “working”          |
| `dot gateway probe`           | Gateway discovery + reachability (local + remote targets)                                              | When you suspect you’re probing the wrong gateway |
| `dot channels status --probe` | Asks the running gateway for channel status (and optionally probes)                                    | When gateway is reachable but channels misbehave  |
| `dot gateway status`          | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error                      | When the service “looks loaded” but nothing runs  |
| `dot logs --follow`           | Live logs (best signal for runtime issues)                                                             | When you need the actual failure reason           |

**Sharing output:** prefer `dot status --all` (it redacts tokens). If you paste `dot status`, consider setting `dot_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  dot models auth setup-token --provider anthropic
  ```
...
```bash
dot models auth paste-token --provider anthropic
dot models status
```
...
- Preferred: `dot logs --follow`
- File logs (always): `/tmp/dot/dot-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$dot_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u dot-gateway[-<profile>].service -n 200 --no-pager`
...
- Or set it directly:
  ```bash
  dot config set gateway.mode local
  ```
...
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  dot config set gateway.mode remote
  dot config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `dot setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.dot/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `dot doctor`
...
- then run `dot sandbox recreate --agent <id>` (or `--all`)
...
- `gateway.mode` must be `local` for `dot gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `dot gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `dot gateway status` and `dot doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `dot_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `dot gateway status` shows a config mismatch**
...
- Fix: rerun `dot gateway install --force` from the same `--profile` / `dot_STATE_DIR` you want the service to use.

**If `dot gateway status` reports service config issues**

- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `dot doctor` to update it (or `dot gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `dot_GATEWAY_TOKEN`) and restart the service.

**If `dot gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/dot` on disk.
...
Symptoms: `pwd` or file tools show `~/.dot/sandboxes/...` even though you
...
dot intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `dot models list` or
  `dot models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${dot_CONFIG_PATH:-$HOME/.dot/dot.json}"
```
...
tail -f "$(ls -t /tmp/dot/dot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
dot logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.dot/agents/<agentId>/sessions/
```
...
dot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
dot channels logout
trash "${dot_STATE_DIR:-$HOME/.dot}/credentials" # if logout can't cleanly remove everything
dot channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/dot/dot-*.log | head -1)" | tail -20
```

### High Memory Usage

dot keeps conversation history in memory.
...
dot now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
- `dot doctor` reports every invalid entry.
- `dot doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `dot logs`, `dot health`, `dot status`, `dot gateway status`, and `dot gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/dot/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `dot models status` (CLI).
...
5. Run `dot channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. dot scrubs/normalizes tool
...
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/dot/dot/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
dot gateway status
dot gateway stop
# Or: launchctl bootout gui/$UID/bot.molt.gateway (replace with bot.molt.<profile>; legacy com.dot.* still works)
```
...
Ensure the global `dot` CLI is installed and matches the app version:

```bash
dot --version
npm install -g dot@<version>
```
...
#   ${dot_CONFIG_PATH:-$HOME/.dot/dot.json} -> { logging: { level: "trace" } }
#
# Then run verbose commands to mirror debug output to stdout:
dot gateway --verbose
dot channels login --verbose
```
...
| Log                               | Location                                                                                                                                                                                                                                                                                                                    |
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gateway file logs (structured)    | `/tmp/dot/dot-YYYY-MM-DD.log` (or `logging.file`)                                                                                                                                                                                                                                                                 |
| Gateway service logs (supervisor) | macOS: `$dot_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.dot/logs/...`; profiles use `~/.dot-<profile>/logs/...`)<br />Linux: `journalctl --user -u dot-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "dot Gateway (<profile>)" /V /FO LIST` |
| Session files                     | `$dot_STATE_DIR/agents/<agentId>/sessions/`                                                                                                                                                                                                                                                                            |
| Media cache                       | `$dot_STATE_DIR/media/`                                                                                                                                                                                                                                                                                                |
| Credentials                       | `$dot_STATE_DIR/credentials/`                                                                                                                                                                                                                                                                                          |
...
# dot gateway uninstall

trash "${dot_STATE_DIR:-$HOME/.dot}"
dot channels login         # re-pair WhatsApp
dot gateway restart           # or: dot gateway
```
...
1. Check logs first: `/tmp/dot/` (default: `dot-YYYY-MM-DD.log`, or your configured `logging.file`)
