...
- During heartbeat runs, dot treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `dot sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, dot skips the heartbeat run entirely (no model call).
...
```bash
dot system event --text "Check for urgent follow-ups" --mode now
```
