---
summary: "Direct `dot agent` CLI runs (with optional delivery)"
read_when:
  - Adding or modifying the agent CLI entrypoint
title: "Agent Send"
---

# `dot agent` (direct agent runs)

`dot agent` runs a single agent turn without needing an inbound chat message.
...
- Optional delivery back to a channel with `--deliver` + `--channel` (target formats match `dot message --target`).
...
```bash
dot agent --to +15555550123 --message "status update"
dot agent --agent ops --message "Summarize logs"
dot agent --session-id 1234 --message "Summarize inbox" --thinking medium
dot agent --to +15555550123 --message "Trace logs" --verbose on --json
dot agent --to +15555550123 --message "Summon reply" --deliver
