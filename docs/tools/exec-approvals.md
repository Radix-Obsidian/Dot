...
- **gateway host** → `dot` process on the gateway machine
...
`~/.dot/exec-approvals.json` directly.

CLI: `dot approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
