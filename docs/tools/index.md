---
summary: "Agent tool surface for dot (browser, canvas, nodes, message, cron) replacing legacy `dot-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `dot-*` skills
title: "Tools"
---

# Tools (dot)

dot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `dot-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `dot.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, dot logs a warning and ignores the allowlist so core tools stay available.
...
- `host=node` can target a macOS companion app or a headless node host (`dot node run`).
...
- Requires a Brave API key (recommended: `dot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated dot-managed browser.
...
- Quick smoke: `dot nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `dot gateway` restart in-place)
