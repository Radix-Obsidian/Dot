---
summary: "Chrome extension: let dot drive your existing Chrome tab"
read_when:
...
The dot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate dot-managed Chrome profile.
...
dot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the dot release (npm package) as static files. There is no separate “build” step.

After upgrading dot:

- Re-run `dot browser extension install` to refresh the installed files under your dot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

dot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want dot to control.
...
- `ON`: attached; dot can drive that tab.
...
`dot browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `dot browser extension install` first to copy the extension to a stable location under your dot state directory.
...
- **This is not isolated** like the dedicated dot-managed profile.
