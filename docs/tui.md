...
```bash
dot tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Check the Gateway logs: `dot logs --follow`.
- Confirm the agent can run: `dot status` and `dot models status`.
...
- No agents in picker: check `dot agents list` and your routing config.
