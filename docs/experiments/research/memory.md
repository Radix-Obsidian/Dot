...
read_when:
  - Designing workspace memory (~/.dot/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep dot integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.dot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.dot/workspace` as canonical human-readable memory.
...
```
~/.dot/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in dot**, but keep a separable core library.

### Why integrate into dot?
...
- You want the agent itself to call the tools:
  - `dot memory recall "…" --k 25 --since 30d`
  - `dot memory reflect --since 7d`
...
Pragmatic take for `~/.dot/workspace`:
