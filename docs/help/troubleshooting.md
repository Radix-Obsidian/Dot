...
You can also set `dot_VERBOSE=1` instead of the flag.
...
### `docs.dot.ai` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.dot.ai` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.dot.ai` to the allowlist, then retry.
...
- Check the allowlist: `dot config get agents.defaults.models`
...
If you can, include the relevant log tail from `dot logs --follow`.
