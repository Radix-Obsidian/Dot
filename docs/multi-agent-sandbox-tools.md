...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `dot sandbox explain`.
...
Legacy `agent.*` configs are migrated by `dot doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
4. **Monitor logs:**
   ```exec
   tail -f "${dot_STATE_DIR:-$HOME/.dot}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
