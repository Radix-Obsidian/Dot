---
summary: "First-run onboarding flow for dot (macOS app)"
read_when:
...
- Writes credentials to `~/.dot/credentials/oauth.json`
...
The app can install the global `dot` CLI via npm/pnpm so terminal
...
On the first agent run, dot bootstraps a workspace (default `~/.dot/workspace`):
...
```bash
dot webhooks gmail setup --account you@gmail.com
```
