---
summary: "dot on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up dot on a Raspberry Pi
  - Running dot on ARM devices
...
Run a persistent, always-on dot Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most dot features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled dot

# Enable if not
sudo systemctl enable dot

# Start on boot
sudo systemctl start dot
```
...
```bash
# Check logs
journalctl -u dot --no-pager -n 100

# Common fix: rebuild
cd ~/dot  # if using hackable install
npm run build
sudo systemctl restart dot
```
