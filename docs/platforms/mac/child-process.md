...
service via the external `dot` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`dot_PROFILE`; legacy `com.dot.*` is supported).
