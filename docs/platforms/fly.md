---
title: Fly.io
description: Deploy dot on Fly.io
---

# Fly.io Deployment

**Goal:** dot Gateway running on a [Fly.io](https://fly.io) machine with persistent storage, automatic HTTPS, and Discord/channel access.
...
| `internal_port = 3000`         | Must match `--port 3000` (or `dot_GATEWAY_PORT`) for Fly health checks |
| `memory = "2048mb"`            | 512MB is too small; 2GB recommended                                         |
| `dot_STATE_DIR = "/data"` | Persists state on the volume                                                |
...
- Non-loopback binds (`--bind lan`) require `dot_GATEWAY_TOKEN` for security.
- Treat these tokens like passwords.
- **Prefer env vars over config file** for all API keys and tokens. This keeps secrets out of `dot.json` where they could be accidentally exposed or logged.
...
**Note:** With `dot_STATE_DIR=/data`, the config path is `/data/dot.json`.
...
Or visit `https://my-dot.fly.dev/`

Paste your gateway token (the one from `dot_GATEWAY_TOKEN`) to authenticate.
...
**Fix:** Ensure `internal_port` matches the gateway port (set `--port 3000` or `dot_GATEWAY_PORT=3000`).
...
If using `--allow-unconfigured`, the gateway creates a minimal config. Your custom config at `/data/dot.json` should be read on restart.

Verify the config exists:

```bash
fly ssh console --command "cat /data/dot.json"
```
...
echo '{"your":"config"}' | fly ssh console -C "tee /data/dot.json"

# Or use sftp
fly sftp shell
> put /local/path/config.json /data/dot.json
```

**Note:** `fly sftp` may fail if the file already exists. Delete first:

```bash
fly ssh console --command "rm /data/dot.json"
```
...
**Fix:** Ensure `dot_STATE_DIR=/data` is set in `fly.toml` and redeploy.
...
```bash
# List current IPs
fly ips list -a my-dot

# Release public IPs
fly ips release <public-ipv4> -a my-dot
fly ips release <public-ipv6> -a my-dot
...
fly ips allocate-v6 --private -a my-dot
```
