...
Deploy dot on Railway with a one-click template and finish setup in your browser.
...
- `https://<your-railway-domain>/dot` — Control UI
...
- `dot_STATE_DIR=/data/.dot` (recommended)
- `dot_WORKSPACE_DIR=/data/workspace` (recommended)
- `dot_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your dot state + workspace so you can migrate to another host without losing config or memory.
