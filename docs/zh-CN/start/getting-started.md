...
- **推荐：** Brave Search API 密钥用于网络搜索。最简单的方式：
  `dot configure --section web` （存储 `tools.web.search.apiKey`）。
  参见 [网络工具](/tools/web)。
...
```bash
curl -fsSL https://dot.bot/install.sh | bash
```
...
- OAuth 凭据（旧版导入）： `~/.dot/credentials/oauth.json`
- 认证配置文件（OAuth + API 密钥）： `~/.dot/agents/<agentId>/agent/auth-profiles.json`
...
如果您尚未进行全局安装，请通过以下方式运行上手引导步骤 `pnpm dot ...` （从仓库中）。
...
如果 `dot health` 显示"未配置认证"，请返回向导设置 OAuth/密钥认证——智能体在没有认证的情况下将无法响应。

提示： `dot status --all` 是最佳的可粘贴只读调试报告。
健康探针： `dot health` （或 `dot status --deep`）向运行中的 Gateway 请求健康快照。
