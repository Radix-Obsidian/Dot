...
<p align="center">
  ...
  <a href="https://github.com/dot/dot/releases">版本发布</a> ·
  <a href="/">文档</a> ·
  <a href="/start/dot">dot 助手设置</a>
</p>

dot 将 WhatsApp（通过 WhatsApp Web / Baileys）、Telegram（Bot API / grammY）、Discord（Bot API / channels.discord.js）和 iMessage（imsg CLI）桥接至编程智能体，例如 [Pi](https://github.com/badlogic/pi-mono)。插件可添加 Mattermost（Bot API + WebSocket）等更多渠道支持。
dot 同时也驱动着 dot 助手。

## 从这里开始

- **从零开始全新安装：** [快速入门](/start/getting-started)
- **引导式设置（推荐）：** [向导](/start/wizard) （`dot onboard`）
- **打开仪表盘（本地 Gateway）：** http://127.0.0.1:18789/（或 http://localhost:18789/）

如果 Gateway 运行在同一台计算机上，该链接会立即打开浏览器控制界面。如果无法打开，请先启动 Gateway： `dot gateway`.
...
大多数操作通过 **Gateway** （`dot gateway`进行，它是一个长期运行的单进程，负责管理渠道连接和 WebSocket 控制面。
...
- **优先回环**：Gateway WS 默认监听 `ws://127.0.0.1:18789`.
  ...
  - 如需 Tailnet 访问，请运行 `dot gateway --bind tailnet --token ...` （非回环绑定时必须提供令牌）。
- **节点**：通过 WebSocket 连接到 Gateway（根据需要使用局域网/Tailnet/SSH）；旧版 TCP 桥接已弃用/移除。
- **Canvas 主机**：HTTP 文件服务器运行在 `canvasHost.port` （默认 `18793`），提供 `/__dot__/canvas/` 用于节点 WebView；请参阅 [Gateway 配置](/gateway/configuration) （`canvasHost`）。
...
之后在 npm 安装和 git 安装之间切换很简单：安装另一种方式并运行 `dot doctor` 以更新 Gateway 服务入口点。
...
如果尚未进行全局安装，请通过以下方式运行上手引导步骤 `pnpm dot ...` （在仓库目录中执行）。
...
- 如果你 **不做任何操作**，dot 将使用内置的 Pi 二进制文件以 RPC 模式运行，并采用按发送者区分的会话。
...
**dot = CLAW + TARDIS** — 因为每只太空龙虾都需要一台时空机器。
