---
summary: "Use Synthetic's Anthropic-compatible API in dot"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. dot registers it as the
...
```bash
dot onboard --auth-choice synthetic-api-key
```
...
Note: dot's Anthropic client appends `/v1` to the base URL, so use
