---
summary: "Use MiniMax M2.1 in dot"
read_when:
  - You want MiniMax models in dot
...
```bash
dot plugins enable minimax-portal-auth  # skip if already loaded.
dot gateway restart  # restart if gateway is already running
dot onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/dot/dot/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `dot.json`:
...
- Use `dot models list` and `dot models set minimax/MiniMax-M2.1` to switch.
...
- Running `dot configure` and selecting **MiniMax M2.1**, or
