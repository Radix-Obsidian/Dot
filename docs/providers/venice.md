---
summary: "Use Venice AI privacy-focused models in dot"
read_when:
  - You want privacy-focused inference in dot
...
```bash
dot onboard --auth-choice venice-api-key
```
...
```bash
dot chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, dot shows all available Venice models. Pick based on your needs:
...
```bash
dot models set venice/claude-opus-45
dot models set venice/llama-3.3-70b
```

List all available models:

```bash
dot models list | grep venice
```

## Configure via `dot configure`
...
dot automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
dot chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
dot chat --model venice/claude-opus-45

# Use uncensored model
dot chat --model venice/venice-uncensored

# Use vision model with image
dot chat --model venice/qwen3-vl-235b-a22b

# Use coding model
dot chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `dot models list` to see currently available models. Some models may be temporarily offline.
