...
- Jobs persist under `~/.dot/cron/` so restarts don’t lose schedules.
...
dot cron list
dot cron run <job-id> --force
dot cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.dot/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `dot cron add/edit` or the cron
...
- Job store: `~/.dot/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.dot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
dot cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
dot cron edit <jobId> --agent ops
dot cron edit <jobId> --clear-agent
```
...
```bash
dot system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`dot system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `dot_SKIP_CRON`.
