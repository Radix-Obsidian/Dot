...
metadata: { "dot": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == dot `sessions_spawn`
- **File I/O** == dot `read`/`write`
- **Remote fetch** == dot `web_fetch` (or `exec` with curl when POST is required)
