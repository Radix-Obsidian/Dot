...
dot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
dot channels login --channel zalouser
...
```bash
dot message send --channel zalouser --target <threadId> --message "Hello from dot!"
```
...
```bash
dot channels login --channel zalouser              # Login via QR
dot channels login --channel zalouser --account work
dot channels status --probe
dot channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
dot directory self --channel zalouser
dot directory peers list --channel zalouser --query "name"
dot directory groups list --channel zalouser --query "work"
dot directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
dot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
dot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `dot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
dot channels login --channel zalouser --account work
dot message send --channel zalouser --account work --target <id> --message "Hello"
