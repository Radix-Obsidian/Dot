...
`dot.invoke --each`) without adding new dot code per workflow.
...
This extension depends on dot internal modules (the embedded agent runner).
It is intended to ship as a **bundled** dot extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.dot/extensions` as a standalone plugin directory.
