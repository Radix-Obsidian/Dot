...
This document describes how the dot ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`dot acp` exposes an ACP agent over stdio and forwards prompts to a running
dot Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a dot Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `dot acp` over stdio.

Example config:

```bash
dot config set gateway.remote.url wss://gateway-host:18789
dot config set gateway.remote.token <token>
```

Example run:

```bash
dot acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
dot acp --session agent:main:main
dot acp --session agent:design:main
dot acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “dot ACP” to start a thread.

## Execution Model

- ACP client spawns `dot acp` and speaks ACP messages over stdio.
...
```bash
dot acp --session agent:main:main
dot acp --session-label "support inbox"
dot acp --reset-session
```
...
`dot acp` resolves the Gateway URL and auth from CLI flags or config:
