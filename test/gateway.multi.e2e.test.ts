...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `dot-e2e-${name}-`));
  const configDir = path.join(homeDir, ".dot");
  await fs.mkdir(configDir, { recursive: true });
  const configPath = path.join(configDir, "dot.json");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          DOT_CONFIG_PATH: configPath,
          DOT_STATE_DIR: stateDir,
          DOT_GATEWAY_TOKEN: "",
          DOT_GATEWAY_PASSWORD: "",
          DOT_SKIP_CHANNELS: "1",
          DOT_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        DOT_GATEWAY_TOKEN: inst.gatewayToken,
        DOT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          DOT_GATEWAY_PORT: String(gwA.port),
          DOT_GATEWAY_TOKEN: gwA.gatewayToken,
          DOT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          DOT_GATEWAY_PORT: String(gwB.port),
          DOT_GATEWAY_TOKEN: gwB.gatewayToken,
          DOT_GATEWAY_PASSWORD: "",
        }),
